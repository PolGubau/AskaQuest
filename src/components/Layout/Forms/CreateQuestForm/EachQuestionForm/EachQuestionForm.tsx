...
export default function EachQuestionForm ({ saveQuestion }: any) {
  ...
  const changeAnswers = (e: any) => {
    ...
    if (question.answers[0] !== '' && question.answers[1] !== '' && question.answers[2] !== '') {
      setAnswersDone(true)
    }
  }
          ...
          <section className={styles.questionFooter}>
            <div className={styles.correctOneSection}>
              <div>
                ...
                <input disabled={!answersDone} type="radio" value={question.answers[0]} id="Answer1radio"
                  onChange={changeSolution} name="AnswerRadio" />
                <label htmlFor="Answer1radio">{question.answers[0].length > 1 ? question.answers[0] : 'Answer 1'}</label>
              </div>
              <div>
                <input
                  disabled={!answersDone}
                  type="radio"
                  value={question.answers[1]} id="Answer2radio"
                  onChange={changeSolution}
                  name="AnswerRadio"
                />

                <label
                  htmlFor="Answer2radio">
                  {question.answers[1].length > 1 ? question.answers[1] : 'Answer 2'}
                </label>
              ...
              <div>
                <input
                  ...
                  name="AnswerRadio" />
                <label
                  htmlFor="Answer3radio">
                  {question.answers[2].length > 1 ? question.answers[2] : 'Answer 3'}
                </label>
              </div>
            </div>
            <div
